...
from monitoring.freshness_check import check_manifest_freshness, check_rows_freshness
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    ingest_status, ingest_detail = check_rows_freshness(rows, sla_hours=float(os.environ.get("FRESHNESS_SLA_HOURS", "24")))
    log(f"freshness_ingest_check={ingest_status} {json.dumps(ingest_detail, ensure_ascii=False)}")
    ...
    validated_cleaned, validation_detail = validate_cleaned_rows(cleaned)
    log(f"pydantic_validation={('PASS' if validation_detail['passed'] else 'FAIL')} {json.dumps(validation_detail, ensure_ascii=False)}")
    if not validation_detail["passed"] and not args.skip_validate:
        log("PIPELINE_HALT: pydantic validation failed.")
        return 2
    ...
    log(f"freshness_publish_check={status} {json.dumps(fdetail, ensure_ascii=False)}")
...
def cmd_embed_internal(cleaned_csv: Path, *, run_id: str, log) -> bool:
    ...
    model_name = os.environ.get("EMBEDDING_MODEL", "local-hash")
    emb = build_embedding_function(model_name=model_name, log=log)
